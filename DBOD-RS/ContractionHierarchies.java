...
class Distance implements Serializable {
    // Ids are made so that we dont have to reinitialize everytime the distance
    // value to infinity.
...
// in this ids are made for the same reason, to not have to reinitialize
// processed variable for every query in bidirectional dijkstra.
class Processed implements Serializable {
...
class Vertex implements Serializable {
    ...
    // parameters for computing importance according to which we will contract the
    // vertices. Vertex wih least importance wil be contracted first.
...
class PQIMPcomparator implements Comparator<Vertex> {
...
class PriorityQueueComp implements Comparator<Vertex> {
    public int compare(Vertex node1, Vertex node2) {
...
class PreProcess {
    ...
    private void computeImportance(Vertex[] graph) {
        ...
        for (int i = 0; i < graph.length; i++) {
            graph[i].edgeDiff = (graph[i].inEdges.size() * graph[i].outEdges.size()) - graph[i].inEdges.size()
                - graph[i].outEdges.size();
    ...
    private void contractNode(Vertex[] graph, Vertex vertex, int contractId) {
        ...
        calNeighbors(graph, vertex.inEdges, vertex.outEdges); // update the given vertex's neighbors about that the
        // given vertex is contracted.
        ...
        for (int i = 0; i < inEdges.size(); i++) {
            ...
            dijkstra(graph, inVertex, max, contractId, i); // finds the shortest distances from the inVertex to all
            // the outVertices.
            ...
            for (int j = 0; j < outEdges.size(); j++) {
                ...
                if (graph[outVertex].distance.contractId != contractId || graph[outVertex].distance.sourceId != i
    ...
    private boolean checkId(Vertex[] graph, int source, int target) {
        if (graph[source].distance.contractId != graph[target].distance.contractId
...
// priorityQueue(min heap) for bidirectional dijkstra algorithms.(for forward
// search)
class forwComparator implements Comparator<Vertex>, Serializable {
...
// priorityQueue(min heap) for bidirectional dijkstra algorithms.(for backward
// search)
class revComparator implements Comparator<Vertex>, Serializable {
    public int compare(Vertex vertex1, Vertex vertex2) {
...
class BidirectionalDijkstra implements Serializable {
        ...
        if (str == "f") {
            ...
            for (int i = 0; i < vertexList.size(); i++) {
                ...
                if (graph[vertex].orderPos < graph[temp].orderPos) {
                    if (graph[vertex].distance.forwqueryId != graph[temp].distance.forwqueryId
            ...
            for (int i = 0; i < vertexList.size(); i++) {
                ...
                if (graph[vertex].orderPos < graph[temp].orderPos) {
                    if (graph[vertex].distance.revqueryId != graph[temp].distance.revqueryId
