...
public class Sdiscount {
    ...
    public static void main(String[] args) {
        ...
        int linesnumber = Integer.parseInt(args[3]);
       ...
       //read data
        ...
        try {
            BufferedReader reader = new BufferedReader(new FileReader("1.13 5w_merge.csv"));
        ...
        double[][] matrix_odistance= new double [listDistance.size()][(linesnumber)];
        for (String[] record : listDistance) {

            for (int j = 0; j < (linesnumber); j++) {
                matrix_odistance[cnt][j] = Double.valueOf(record[j].toString());
            }
            cnt++;
        }
        System.out.println( (linesnumber)+ " lines");
        ...
        count = 0;
        try {
            BufferedReader reader = new BufferedReader(new FileReader("1.13 5w_order.csv"));
            while ((line = reader.readLine()) != null) {
                if (count == linesnumber) {
                    break;
                }
        ...
        count = 0;
        try {
            BufferedReader reader = new BufferedReader(new FileReader("1.13 5w_vehicle.csv"));
            while ((line = reader.readLine()) != null) {
                if (count == (linesnumber)) {
                    break;
                }
        ...
        profit =new double[map_orders.size()][2];

        for (int k = 0; k < map_orders.size(); k++) {
            boolean distrue = false;
            int k_actual = map_orders.get(k);
            Double discount = (Double) new_list.get(k_actual)[6];
            Double tol = (Double) new_list.get(k_actual)[7];
            for (int j = 0; j < matrix_distance[0].length; j++) {
                double pick_dis = matrix_distance[k][j];
            ...
            if(distrue == true){
                profit[k][0] = discount;
                profit[k][1] = getProfit(matrix_distance, map_orders, map_vehicles, new_list, new_list1, discount, 0, given_dis);
            }
        }

        ebmaxprofit[0]=getProfit(matrix_distance, map_orders, map_vehicles, new_list, new_list1, getMax(profit),1, given_dis);
        max_profit+=ebmaxprofit[0];
        ...
        try {
            ...
            writer_profit.write("BRU " +",");
            writer_profit.write(ebmaxprofit[0] + ","  );
